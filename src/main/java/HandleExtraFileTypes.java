...
public class HandleExtraFileTypes extends ImagePlus implements PlugIn {
    ...
    private static boolean checkForLoci() {
        ...
        try {
            lociPresent = IJ.getClassLoader().loadClass(
                    "loci.plugins.LociImporter") != null;
    ...
    private Object tryOpen(final String directory, String name, final String path) {
        ...
        if (name.endsWith(
                ".ipl") || (buf[0] == 105 && buf[1] == 105 && buf[2] == 105 && buf[3] == 105) || (buf[0] == 109 && buf[1] == 109 && buf[2] == 109 && buf[3] == 109)) {
        ...
        if (name.endsWith(".nii") || name.endsWith(".nii.gz") || name.endsWith(
                ".nii.z")) {
            return tryPlugIn("Nifti_Reader", path);
        }
        ...
        if (name.equals("ser") || name.equals("fid") || name.equals("2rr") || name.equals("2ii") || name.equals(
                "3rrr") || name.equals("3iii") || name.equals("2dseq")) {
        ...
        if (name.endsWith(
                ".unc") || (buf[3] == 117 && buf[7] == -127 && buf[11] == 36 && buf[14] == 32 && buf[15] == -127)) {
        ...
        if (name.endsWith(".mrc") || name.endsWith(".rec") || name.endsWith(
                ".st") || name.endsWith(".tmg")) {
        ...
        if (name.endsWith(".xml") || name.endsWith(".xml.gz")) {
            ...
            if (name.endsWith("z")) {
                ...
                b = new byte[132];
                try {
                    gz = new GZIPInputStream(new BufferedInputStream(new FileInputStream(
                            path)));
                    gz.read(b, 0, 132);
        ...
        if (name.toLowerCase().endsWith(".bin") || name.toLowerCase().endsWith(
                ".bin.gz")) {
        ...
        if (name.endsWith(".obj") || name.endsWith(".dxf") || name.endsWith(
                ".stl")) {
        ...
        try {
            ...
            if (vaa3dCookie.equals("raw_image_stack_by_hpeng") || vaa3dCookie.equals(
        ...
        try {
            ...
            if (name.matches("[a-z]\\d{7}.isq;\\d+") || isqMagic.equals(
                    "CTDATA-HEADER_V1")) {
    ...
    private ImagePlus openImage(final String directory, final String name, final String path) {
        ...
        if (o == null && (IJ.getVersion().compareTo(
                "1.38j") < 0 || !IJ.redirectingErrorMessages()) && (new File(path).exists())) {
