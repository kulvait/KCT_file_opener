...
public class DatFileInfo {
    ...
    DatFileInfo(File f) {
        ...
        try {
            ...
            CharsetDecoder decoder = StandardCharsets.ISO_8859_1.newDecoder().onMalformedInput(
            ...
            {
                System.out.printf(
                        "Invalid DAT: firstLine=%s shall have %d elements but has %d\n", firstLine, 3 + DIMCOUNT,
                        fld.length);
                validDAT = false;
                return;
            }
            if (DIMCOUNT > 16) {
                System.out.printf(
                        "Invalid DAT: DIMCOUNT=%d shall not be more than 16\n", DIMCOUNT);
                validDAT = false;
                return;
            }
            switch (fld[2]) {
                ...
                default:
                    System.out.printf(
                            "Invalid DAT: elementType=%s is not supported or known", fld[2]);
                    validDAT = false;
                    return;
            }
            System.out.printf("Openning DAT: first line %s type %s.\n", firstLine, elementType.toString());
    ...
    public DenDataType getElementType() {
        return elementType;
    }
